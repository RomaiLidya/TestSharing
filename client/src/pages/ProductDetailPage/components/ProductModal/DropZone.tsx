...
interface Props {
  ...
  handleOnDelete: (id: number) => React.MouseEventHandler;
}
...
const DropZone: FC<Props> = props => {
  ...
  const { getRootProps, getInputProps, imageUrl, acceptedFiles, handleOnDelete } = props;
    ...
    <Fragment>
      ...
      <aside className={classes.thumbsContainer}>
        {imageUrl.map((value, index) => (
          <Badge
            color={undefined}
            badgeContent={<HighlightOffIcon color='error' onClick={handleOnDelete(value.id)} className={classes.deleteIcon} />}
          >
            <div className={classes.thumb} key={index + 1}>
              <div className={classes.thumbInner}>
                <img src={value.url} className={classes.img} />
              </div>
            </div>
          </Badge>
        ))}
